...
- 主开关与安全重置（延迟显示、二次确认）。
- 按站点持久化设置；性能缓存避免重复解析。
- 全局评分机制：面板顶部提供全局分数阈值（默认 15）。各筛选项命中时贡献分数——关键字（每项默认 5 分）、更新/创建/安装量（每类默认 20 分）、作者（每位作者可单独设置分数，默认 20 分）。在默认值下，任一非关键字条件命中即隐藏；用户可将各类或作者分数调低，使多个条件累计达到阈值后再隐藏。
- 支持负分抵消：关键字与作者都支持负分，可用于抵消隐藏，实现更细粒度的组合控制。
- 关键字过滤：列表管理、逐项开关、分数可编辑；范围（标题/描述/标题+描述）；大小写敏感与正则（`/regex/flags`）；支持负分用于抵消隐藏。
...
- 调试开关：在统计旁提供开关，一键互换“显示/隐藏”的项目，便于确认与调试筛选结果。
